...
extension SameLayerRenderViewController: WKScriptMessageHandler {
    func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        ...
        if nativeRenderViewArr.isEmpty {
            ...
            for item in webView.getAllSubViewss() {
        ...
        if message.name == messageHandlerHandler {
            switch type {
            case "input":
                nativeRenderViewArr[index].removeAllSubviews()
            ...
            case "image":
                nativeRenderViewArr[index].removeAllSubviews()
                imageView.frame = nativeRenderViewArr[index].frame
                nativeRenderViewArr[index].addSubview(imageView)
            default:
                break
            }
        }
...
extension SameLayerRenderViewController: WKUIDelegate {
    public func webView(_ webView: WKWebView, runJavaScriptAlertPanelWithMessage message: String, initiatedByFrame frame: WKFrameInfo, completionHandler: @escaping () -> Void) {
        ...
        let okAciton = UIAlertAction(title: "确定1", style: .default) { _ in
            completionHandler()
        }
...
extension UIView {
    ...
    /// 获取所有子视图
    fileprivate func getAllSubViewss() -> [UIView] {
        UIView.allSubviews = []
