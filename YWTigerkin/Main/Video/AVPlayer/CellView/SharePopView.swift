...
class SharePopView:UIView {
    ...
    func initSubView() {
        ...
        let label = UILabel.init(frame: CGRect.init(origin: .zero, size: CGSize.init(width: YWConstant.screenWidth, height: 35)))
        ...
        let itemWidth = 68
        let topScrollView = UIScrollView.init(frame: CGRect.init(x: 0, y: 35, width: YWConstant.screenWidth, height: 90))
        ...
        let bottomScrollView = UIScrollView.init(frame: CGRect.init(x: 0, y: 135, width: YWConstant.screenWidth, height: 90))
    ...
    @objc func onShareItemTap(sender:UITapGestureRecognizer) {
        ...
        UIApplication.shared.openURL(URL.init(string: "https://github.com/")!)
        dismiss()
    }
