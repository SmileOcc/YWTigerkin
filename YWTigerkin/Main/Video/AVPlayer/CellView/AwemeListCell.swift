...
class AwemeListCell: UITableViewCell {
    ...
    override func layoutSubviews() {
        ...
        musicName.snp.makeConstraints { make in
            ...
            make.width.equalTo(YWConstant.screenWidth/2)
            make.height.equalTo(20)
        }
        desc.snp.makeConstraints { make in
            make.left.equalTo(self).offset(10)
            make.bottom.equalTo(self.musicIcon.snp.top).inset(-5)
            make.width.lessThanOrEqualTo(YWConstant.screenWidth / 5 * 3)
        }
        nickName.snp.makeConstraints { make in
            make.left.equalTo(self).offset(10)
            make.bottom.equalTo(self.desc.snp.top).inset(-5)
            make.width.lessThanOrEqualTo(YWConstant.screenWidth / 4 * 3 + 30)
        }
    ...
    func initData(aweme:YWDouYinVideoModel) {
        self.aweme = aweme
        //odd测试数据
        playerView.setPlayerSourceUrl(url: aweme.url ?? "")
        nickName.text = "昵称1"
        desc.text = aweme.url ?? "描述"
        musicName.text = "music-name"
        favoriteNum.text = String.formatCount(count: 10)
        commentNum.text = String.formatCount(count: 100)
        shareNum.text = String.formatCount(count: 1000)
...
extension AwemeListCell {
    ...
    func startLoadingPlayItemAnim(_ isStart:Bool = true) {
        if isStart {
            ...
            scaleAnim.toValue = 1.0 * YWConstant.screenWidth
...
extension AwemeListCell: AVPlayerUpdateDelegate {
    ...
    func onPlayItemStatusUpdate(status: AVPlayerItem.Status) {
        ...
        case .readyToPlay:
            ...
            isPlayerReady = true
            musicAlum.startAnimation(rate: CGFloat( 2100))
            onPlayerReady?()
            break
        case .failed:
            startLoadingPlayItemAnim(false)
